...
모든 작업은 GitHub Issue를 먼저 만들고 진행한다. 이슈 없이 작업하지 않는다.

```
1. GitHub Issue 생성 (목적, 변경 내용, 관련 파일)
2. 브랜치 생성: {이슈번호}/{타입}/{설명}
3. 작업 + 테스트
4. PR 생성 → 머지
5. 이슈 Close
...
**이유**: 모든 변경 이력을 추적하고, 나중에 "왜 이렇게 바꿨는지" 확인할 수 있도록.

### 이슈 라벨 규칙

| 라벨 | 색상 | 용도 |
|------|------|------|
| `core` | 빨강 | 수익률/매매 로직에 직접 영향 (신중히) |
| `UI/UX` | 핑크 | 디자인/사용자 경험 개선 |
| `bug` | 빨강 | 버그 수정 |
| `enhancement` | 민트 | 새 기능/개선 |
| `test` | 연청 | 테스트 관련 |
| `phase:N` | 각 색상 | 로드맵 Phase 구분 |

**주요 원칙:**
- 수익률에 영향가는 이슈는 반드시 `core` 태그 → 충분한 검토 후 작업
- UI 변경은 `UI/UX` 태그
- 하나의 이슈에 여러 태그 가능 (예: `core, phase:4`)

### 작업 단위

- 기능 추가, 버그 수정, 리팩토링 모두 이슈 생성 후 진행
- 사소한 수정(오타, 설정값 변경)도 커밋 메시지에 근거 기록
- 여러 작업을 한 이슈에 묶지 말고, 작업 단위로 분리

### 예외: Hotfix

서버 장애, 빌드 에러 등 **즉시 수정이 필요한 경우**는 브랜치/PR 없이 main에 직접 커밋 가능.

- 커밋 메시지에 `hotfix:` 접두사 사용
- **사후에 반드시 이슈 생성** → 원인/해결/커밋 해시 기록 → 즉시 Close
- 커밋 메시지에 이슈번호 포함: `hotfix: 에러 수정 (#이슈번호)`
- 예: `hotfix: DataFrame bool 변환 에러 (#106)`
