...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([bal, pos, hist, mkt, trades, strats]) => {
      setBalance(bal);
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        <div className="card-title">자산 추이 (30일)</div>
        {history.length > 0 ? (
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={history}>
              ...
              />
              <Area type="monotone" dataKey="total_asset_value_krw" stroke="#4a9eff" fill="url(#assetGradient)" />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
