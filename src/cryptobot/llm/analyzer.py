...
{{
  ...
  "coin_strategies": {{
    // (선택) 코인별로 **다른** 전략을 쓰고 싶을 때만 채움. 대부분은 recommended_strategy 하나로 충분.
    // 백테스트 결과상 코인마다 최적 전략이 다를 때 활용.
    // 형식: "KRW-XXX": {{"strategy": "전략명", "params": {{키: 값}}}}
    // 여기 명시되지 않은 코인은 recommended_strategy(기본)를 사용한다.
    // 예: "KRW-ENSO": {{"strategy": "breakout_momentum", "params": {{"entry_period": 10}}}}
    // 비어있어도 OK — 단일 전략으로 충분하면 빈 객체.
  }},
