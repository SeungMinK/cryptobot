...
### 핵심 목표: 자산 성장 (승률이 아님!)
- **승률보다 총자산 증가가 중요합니다**
- 현재 문제: 승률 70%+ 이지만 평균 승 +0.3%, 평균 패 -5% → 손익비 1:16
- 1번 지면 16번 이겨야 본전 → 승률이 높아도 자산이 안 늘어남
- **해결: 이길 때 더 크게 이기도록 ROI 기준을 높여야 합니다**

### ROI 테이블 조절 가이드
- ROI 테이블은 "N분 보유 후 최소 X% 수익이면 매도"
- 현재 120분에 +0.1%면 매도 → 너무 빨리 팔아서 건당 수익 작음
- **추천: 시장 변동성에 따라 ROI 기준을 올리세요**
  - 횡보장: 60분 +1.5%, 120분 +0.5% (적당히)
  - 상승장: 60분 +2.0%, 120분 +1.0% (더 올라갈 여지)
  - 하락장: 60분 +1.0%, 120분 +0.3% (빨리 탈출)
- 손절(-5.5%)에 비해 목표 수익이 최소 1:3 이상이어야 의미 있음
  - 예: 손절 -5% → 목표 최소 +1.5% 이상에서 매도
...
class LLMAnalyzer:
    ...
    def _get_performance_text(self) -> str:
        ...
        rr_ratio = f"1:{avg_loss / avg_win:.0f}" if avg_win > 0 else "N/A"

        lines = [
            f"24시간: {r['total']}건 (승률 {win_rate:.0f}%, 손익 {r['total_pnl'] or 0:+,.0f}원)",
            f"평균 승: +{avg_win:.2f}%, 평균 패: -{avg_loss:.2f}%",
            f"손익비: {rr_ratio} (1번 지면 이걸로 본전치려면 {avg_loss / avg_win:.0f}번 이겨야)" if avg_win > 0 else "",
        ]
        if avg_win > 0 and avg_loss / avg_win > 5:
            lines.append("⚠️ 손익비 불균형! 이길 때 더 크게 이기도록 ROI 기준을 올리세요.")
